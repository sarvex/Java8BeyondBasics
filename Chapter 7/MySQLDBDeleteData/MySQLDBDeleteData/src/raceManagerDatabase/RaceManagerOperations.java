...
public class RaceManagerOperations {
  ...
  public static String CreateAthleteTableDetails() {
    return "`RacerID` INT NOT NULL AUTO_INCREMENT, " + "`Name` VARCHAR(100) NOT NULL, " + "`Age` INT NOT NULL, "
        + "`IsInjured` BIT NOT NULL, " + "`TypeId` INT NOT NULL, " + "`Shoes` VARCHAR(45) NULL, "
        + "`UsesClips` BIT NULL, " + "PRIMARY KEY (`RacerID`)";
  }

  public static String CreateAthleteTypeTableDetails() {
    return "`ID` INT NOT NULL AUTO_INCREMENT, " + "`AthleteType` VARCHAR(45) NOT NULL, " + "PRIMARY KEY (`ID`)";
  }
  ...
  public static String InsertAthleteTypeString(String aType) {
    return "INSERT INTO " + ATHLETE_TYPE_TABLE_NAME + " (`AthleteType`) " + " VALUES('" + aType + "')";
  }
  ...
  public static String InsertAthleteString(Athlete a, int typeId) {
    String athleteInsert = "Insert into Athlete (`Name`, " + "`Age`, " + "`IsInjured`, " + "`TypeId`, " + "`Shoes`, "
        + "`UsesClips`) " + "VALUES " + "('" + a.getName() + "', " + a.getAge() + ", " + a.getIsInjured() + ", "
        + typeId + ", ";
  ...
  public static String UpdateAthleteString(Athlete a) {
    ...
    return "UPDATE " + ATHLETE_TABLE_NAME + " SET name='" + a.getName() + "', age=" + a.getAge() + " WHERE RacerID="
        + a.getRacerId();
  }

  /** Get a query string to delete athlete by racerID
   *
   * @param racerID the id of the racer to delete
   * @return */
