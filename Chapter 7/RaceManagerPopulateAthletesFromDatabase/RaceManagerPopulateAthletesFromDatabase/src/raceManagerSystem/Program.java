...
import raceManagerData.dataFile;
import raceManagerData.raceManagerPopulateAndStore;
...
public class Program {
      ...
      try {
        ...
        case 7:
          if (checkRaceCreated()) {
            ...
            boolean success = raceManagerPopulateAndStore.writeAthletesToFile(dataFile.dataFilePath,
                _isr.getRacersList());
            if (success) {
  ...
  public static void printRacer(String message, Athlete a) {
    ...
    System.out.println(String.format(outputDetails, message, t, a.getName(), Integer.valueOf(a.getAge()), Integer.valueOf(a.getRacerId()),
        a.performRaceActivitySafe(), extra));
    dashes(menuStars);
  }
  ...
  private static ArrayList<Athlete> restoreAthletesFromRS(DBManager dbm) throws SQLException {
    ...
    while (rs.next()) {
      ...
      Map<String, Integer> athleteTypes = ShowDatabaseOperations.getAthleteTypes(dbm);
      if (athleteTypes.get(ShowDatabaseOperations.RUNNER) == type) {
        a = new Runner();
      } else if (athleteTypes.get(ShowDatabaseOperations.BIKER) == type) {
        a = new Biker();
      } else if (athleteTypes.get(ShowDatabaseOperations.SWIMMER) == type) {
        a = new Swimmer();
      }
