...
public class ShowDatabaseOperations {
  public static final String CONFIG_PATH = "C:\\JavaProjects\\RaceManagerPopulateAthletesFromDatabase"
      + "\\src\\ServerInterop\\dataconfig.json";
  public static final String DB_KEY = "racemanagerdb";
  ...
  /*
   * public static void main(String[] args) { try { //create the connector ServerConnector sc = new
   * MySQLServerConnector(CONFIG_PATH, DB_KEY);
   *
   * //create the manager DBManager dbm = new DBManager(sc);
   *
   * //connect dbm.openConnection();
   *
   * Scanner input = new Scanner(System.in); String userEntry; boolean allDone = false; while (!allDone) { int option =
   * printMenu(); switch(option) { case 1: listAllTables(dbm); break; case 2: //verify a table exists
   * System.out.println("Please enter the name of the table for which to verify existence: "); userEntry =
   * input.nextLine(); verifyTableExists(dbm, userEntry); break; case 3: //create a table System.out.println(
   * "Enter 1 to create the Athlete Table, 2 to create the Athlete Type Table:"); userEntry = input.nextLine(); if
   * (userEntry.equals("1")) { createTable(dbm, RaceManagerOperations.ATHLETE_TABLE_NAME ,
   * RaceManagerOperations.CreateAthleteTableDetails()); } else if (userEntry.equals("2")) { createTable(dbm,
   * RaceManagerOperations.ATHLETE_TYPE_TABLE_NAME , RaceManagerOperations.CreateAthleteTypeTableDetails()); } else {
   * System.out.println("Invalid Choice"); } break; case 4: //delete a table System.out.println(
   * "Please enter 1 to delete the Athlete table, 2 to delete the Athlete Type table. "); userEntry = input.nextLine();
   * if (userEntry.equals("1")) { deleteTable(dbm, RaceManagerOperations.ATHLETE_TABLE_NAME); } else if
   * (userEntry.equals("2")) { deleteTable(dbm, RaceManagerOperations.ATHLETE_TYPE_TABLE_NAME); } else {
   * System.out.println("Invalid choice"); } break; case 5: //insert athlete types insertAthleteTypes(dbm);
   * System.out.println("Inserted Athlete Types!"); break; case 6: //insert a new athlete. System.out.println(
   * "Please enter 1 to insert a new Runner, " + "2 to insert a new Biker, or 3 to insert a new Swimmer. "); userEntry =
   * input.nextLine(); Athlete a = null; if (userEntry.equals("1")) { a = new Runner(); } else if
   * (userEntry.equals("2")) { a = new Biker(); } else if (userEntry.equals("3")) { a = new Swimmer(); }
   * buildAthlete(a); insertAthlete(a, dbm); System.out.println("Inserted the athlete: " + a.getName()); break; case 7:
   * printAthletes(dbm); break; case 8: printAthletes(dbm); System.out.println("Please enter the racerID to update: ");
   * userEntry = input.nextLine(); updateAthlete(dbm, Integer.parseInt(userEntry)); System.out.println(
   * "Updated the Athlete Data!"); break; case 9: printAthletes(dbm); System.out.println(
   * "Please enter the racerID to delete: "); userEntry = input.nextLine(); deleteAthlete(dbm,
   * Integer.parseInt(userEntry)); System.out.println("Deleted the Athlete Data!"); default: break; }
   * System.out.println("Would you like to do another operation [y/n]?"); String choice = input.nextLine(); if
   * (choice.toLowerCase().charAt(0) == 'n') { allDone = true; } }
   *
   * //close connection dbm.closeConnection(false); } catch (Exception e) { // TODO need better logging...
   * e.printStackTrace(); } System.out.println("Thank you for using the database manager!"); }
   */
  ...
  public static int printMenu() {
    ...
    while (!valid) {
      try {
        ...
        System.out.println("Enter 9 to delete an athlete: ");
   ...
   *
   * @param a the athlete to set up for insert NOTE: There is no validation in the interest of time and simplicity.
   ...
   * @throws SQLException Note: Records in the DB must contain "Runner, Biker, Swimmer" to work as expected */
  public static void insertAthlete(Athlete a, DBManager dbm) throws SQLException {
    // get the types to get the id
    Map<String, Integer> athleteTypes = getAthleteTypes(dbm);
    int typeID = 0;
  ...
  public static void printAthletes(DBManager dbm) throws SQLException {
    ...
    while (rs.next()) {
      System.out
          .println("RacerID: " + rs.getInt("RacerID") + " Name: " + rs.getString("Name") + " Age: " + rs.getInt("Age"));
    }
    rs.close();
    System.out.println("********************************");
  }
   ...
   * @throws SQLException if it happens */
  public static Map<String, Integer> getAthleteTypes(DBManager dbm) throws SQLException {
    Connection cn = dbm.GetConnection();
  ...
  public static void insertAthleteTypes(DBManager dbm) throws SQLException {
    Map<String, Integer> types = getAthleteTypes(dbm);
    if (types.size() == 0) {
  ...
  public static Athlete getAthlete(DBManager dbm, int racerID) throws SQLException {
    ...
    while (rs.next()) {
      ...
      if (nextID == racerID) {
        ...
        Map<String, Integer> athleteTypes = getAthleteTypes(dbm);
        if (athleteTypes.get(RUNNER) == type) {
          a = new Runner();
        } else if (athleteTypes.get(BIKER) == type) {
          a = new Biker();
        } else if (athleteTypes.get(SWIMMER) == type) {
          a = new Swimmer();
        }
   ...
   *
   * @param dbm the manager
   * @param racerID the id to delete
   * @throws SQLException if it happens */
