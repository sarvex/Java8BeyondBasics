 ...
 *
 * @author sarvex
 * @param <E> the Type of data to store WARNING: THIS CODE IS NOT THREAD SAFE OR MULTIPLE USER CONCURRENT SAFE */
public class IterableBestDataContainer<E> implements Iterable<E> {
  ...
  /** Add an item to the container. Can't add null, can't add past capacity, can't add duplicates
   *
   * @param item to add
  ...
  /** Remove an item that is equal to the passed in item. can't remove a null item, can't remove an item if it is not in
   * the container.
   *
  ...
  /** Find an item by the item itself relies on valid equality being implemented in type T
   *
  ...
  /** Print out all the items in the container. relies on toString() being implemented in the type T
   *
